...
function SpaceHeader({ currentSpace }: Props) {
  ...
  useEffect(() => {
    if (
      authUser &&
      canControlSpace(currentSpace.channel_id, currentSpace.id, authUser)
    ) {
      ...
      <CardHeader className="pb-2 pt-2 flex flex-row items-center justify-between">
        <CardTitle className="flex items-center">
          <Link href={`/channels/${currentSpace.channel?.channel_slug}/spaces`}>
            <h1 className="text-xl">{currentSpace.channel?.channel_name}</h1>
          </Link>
          <ArrowBigRightDash className="mx-2" />
          <Link
            href={`/channels/${currentSpace.channel?.channel_slug}/spaces/${currentSpace.space_slug}`}
          >
            <h1 className="text-xl">{currentSpace.space_name}</h1>
          </Link>
        </CardTitle>

        {spaceControl ? <SpaceContextMenu currentSpace={currentSpace} /> : null}
      </CardHeader>
    </Card>
  )
}
