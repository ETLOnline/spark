...
import { createInvitation, getValidInvitationWithRole, updateJoinedEmail } from "@/src/db/data-access/invite/query"
...
export const VerifyInviteAction = CreateServerAction(
  ...
  async (data: { key: string }) => {
    try {
      ...
      const invitation = await getValidInvitationWithRole(data.key, authUser.email);
