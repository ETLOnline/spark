...
import { createShortcutAction, deleteShortcutAction, getUserShortcutsAction } from "@/src/server-actions/Shortcut/Shortcut"
...
const useShortcut = () => {
  ...
  const [loadingShortcuts,,,getUserShortcuts] = useServerAction(getUserShortcutsAction)
  const [deletingShortcuts,,,deleteShortcutById] = useServerAction(deleteShortcutAction)
  const [creatingShortcuts,,,createShortcutAc] = useServerAction(createShortcutAction)
  ...
  useEffect(()=>{
    if(shortcutList && shortcutList.length > 0){      
      ...
      const spaceShortcuts = updatedShortcuts.filter((shortcut) => shortcut.type === "space")
      const channelShortcuts = updatedShortcuts.filter((shortcut) => shortcut.type === "channel")
      const communityShortcuts = updatedShortcuts.filter((shortcut) => shortcut.type === "community")
      const projectShortcuts = updatedShortcuts.filter((shortcut) => shortcut.type === "project")
  ...
  return {
    ...
    deleteShortcut,
    createShortcut,
    loadingShortcuts: loadingShortcuts || creatingShortcuts || deletingShortcuts,
    shortcutMap,
    availableShortcut
  }
}
