...
const channelSchema = z.object({
  channel_name: z.string().min(1, "Tilte required").max(30, "Tilte is too long"),
...
function CreateChannels() {
  ...
  const [channelFormModelVisibility, setChannelFormModelVisibility] = useAtom(channelStore.channelformModalVisibility)
  const [addChannelLoading, addChannelData, addChannelError, CreateChannel] = useServerAction(CreateChannelAction);
  const [addUpdateChannelLoading, addUpdateChannelData, addUpdateChannelError, UpdateChannel] = useServerAction(UpdateChannelAction);
  const [addDeleteChannelLoading, addDeleteChannelData, addDeleteChannelError, DeleteChannel] = useServerAction(DeleteChannelAction);
  ...
  async function handleUpdateChannel(updatedData: Partial<SelectChannel>) {
    try {
      ...
      const updatedChannel = await UpdateChannel(selectedChannel.id, updatedData)
      if (updatedChannel?.success && updatedChannel.data)
        setChannel((channel) =>
          channel.map((channel) => channel.id === selectedChannel.id
            ? { ...channel, ...updatedChannel.data } : channel)
        )
      setChannelFormModelVisibility(false)
      toast({
          ...
          <div className='grid gap-4 py-4'>
            ...
            <div className='flex flex-col'>
              ...
              <div className="text-right">
                {error.channel_name && <span className="text-red-500 text-sm">{String(error.channel_name.message)}</span>}
              </div>
            </div>
            ...
            <div className="text-right">
              {error.channel_type && <span className="text-red-500 text-sm">{String(error.channel_type.message)}</span>}
            </div>
          </div>

          <DialogFooter>{editChannal === true ?
            <div className='w-full flex justify-between'>
              <Button variant="destructive" type="button" onClick={handleDeleteChannel} loading={addDeleteChannelLoading}>Delete</Button>
              <Button type="submit" loading={addUpdateChannelLoading}>Save</Button>
            </div> :
