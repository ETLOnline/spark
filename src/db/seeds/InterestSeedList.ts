export const interestSeedList = [
  "Web Development", // 0
  "Mobile Development", // 1
  "Open Source", // 2
  "Artificial Intelligence", // 3
  "Machine Learning", // 4
  "Data Science", // 5
  "Cybersecurity", // 6
  "UI/UX Design", // 7
  "Cloud Computing", // 8
  "DevOps", // 9
  "Game/Playable Development", // 10
  "Blockchain", // 11
  "Internet of Things (IoT)", // 12
  "Augmented Reality", // 13
  "Virtual Reality", // 14
  "Software Engineering", // 15
  "Digital Marketing", // 16
  "Graphic Design", // 17
  "Product Management", // 18
  "Entrepreneurship", // 19
  "Startups", // 20
  "Project Management", // 21
  "E-commerce", // 22
  "Tech Blogging", // 23
  "Public Speaking", // 24
  "Photography", // 25
  "Video Editing", // 26
  "Finance & Investing", // 27
  "Health Tech", // 28
  "EdTech", // 29
  "Clean Energy", // 30
  "Social Impact", // 31
  "Freelancing", // 32
  "Remote Work", // 33
  "Career Growth", // 34
  "Personal Development", // 35
  "Reading", // 36
  "Writing", // 37
  "Mentorship", // 38
  "FinTech", // 39
  "Robotics", // 40
  "Quantum Computing", // 41
  "Bioinformatics", // 42
  "Ethical Hacking", // 43
  "Network Security", // 44
  "Big Data", // 45
  "Serverless Architecture", // 46
  "API Development", // 47
  "Database Management", // 48
  "Frontend Frameworks (e.g., React, Vue)", // 49
  "Backend Development (e.g., Node.js, Django)", // 50
  "Wearable Technology", // 51
  "Smart Home Technology", // 52
  "Legal Tech", // 53
  "Agritech", // 54
  "Space Tech", // 55
  "Computer Graphics", // 56
  "Embedded Systems", // 57
  "DevRel (Developer Relations)", // 58
  // Newly Added Non-Tech Interests
  "Travel", // 59
  "Cooking/Baking", // 60
  "Fitness & Wellness", // 61
  "Sports", // 62
  "Music (Playing/Listening)", // 63
  "Art & Culture", // 64
  "Volunteering", // 65
  "Environmentalism", // 66
  "Psychology", // 67
  "Philosophy", // 68
  "History", // 69
  "Languages (Learning New)", // 70
  "Gardening", // 71
  "DIY & Crafts", // 72
  "Podcast Creation/Listening", // 73
  "Meditation & Mindfulness", // 74
  "Financial Literacy", // 75
  "Community Building", // 76
  "Content Creation", // 77
  "Storytelling", // 78
  "Leadership Development", // 79
  "Negotiation Skills", // 80
  "Critical Thinking", // 81
  "Problem Solving", // 82
  "Design Thinking", // 83
  "Mind Mapping", // 84
  "Memory Improvement" // 85
]

export const newInterestsList = [
  "FinTech", // 39
  "Robotics", // 40
  "Quantum Computing", // 41
  "Bioinformatics", // 42
  "Ethical Hacking", // 43
  "Network Security", // 44
  "Big Data", // 45
  "Serverless Architecture", // 46
  "API Development", // 47
  "Database Management", // 48
  "Frontend Frameworks (e.g., React, Vue)", // 49
  "Backend Development (e.g., Node.js, Django)", // 50
  "Wearable Technology", // 51
  "Smart Home Technology", // 52
  "Legal Tech", // 53
  "Agritech", // 54
  "Space Tech", // 55
  "Computer Graphics", // 56
  "Embedded Systems", // 57
  "DevRel (Developer Relations)", // 58
  // Newly Added Non-Tech Interests
  "Travel", // 59
  "Cooking/Baking", // 60
  "Fitness & Wellness", // 61
  "Sports", // 62
  "Music (Playing/Listening)", // 63
  "Art & Culture", // 64
  "Volunteering", // 65
  "Environmentalism", // 66
  "Psychology", // 67
  "Philosophy", // 68
  "History", // 69
  "Languages (Learning New)", // 70
  "Gardening", // 71
  "DIY & Crafts", // 72
  "Podcast Creation/Listening", // 73
  "Meditation & Mindfulness", // 74
  "Financial Literacy", // 75
  "Community Building", // 76
  "Content Creation", // 77
  "Storytelling", // 78
  "Leadership Development", // 79
  "Negotiation Skills", // 80
  "Critical Thinking", // 81
  "Problem Solving", // 82
  "Design Thinking", // 83
  "Mind Mapping", // 84
  "Memory Improvement" // 85
]
