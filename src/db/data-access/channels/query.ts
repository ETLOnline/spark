...
export async function UpdateChannel(channelID: string ,updatedChannelData: Partial<SelectChannel>) {
  try{
    const updatedChannel = await db.update(channelsTable).set(updatedChannelData)
...
export async function DeleteChannel(deletedChannelData: SelectChannel){
  try{
    ...
    const deleteChannel = await db.delete(channelsTable).where(eq(channelsTable.id, deletedChannelData.id));
  } catch (e: any) {
    throw new Error(e.message);
  }
}