...
export const UpdateContact = async (user_id: string, contact_id: string, payload:Partial<SelectUserContact>) => {
    try{
        return await db.update(userContactsTable).set(payload).where(
            and(
